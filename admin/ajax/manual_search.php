...
DB::useDB(B2B_DB);
$query = "select o.*, c.name as company_name, u.smooch_id as email,u.name as user_name, u.contact as contact from b2b_orders as o inner join company as c on o.company_id = c.id  
...
$list = array
(
    "Order ID,User Email,Name,Contact,Company,Restaurant Name,Total Paid,SubTotal,Discount,Company Contribution,Payment,Order Status,Transaction ID,Date Completed"
);
...
foreach($result as $order) {
    ...
    $output .= '<td>' . $order['user_name'] . '</td>';
    $arr[2] = $order['user_name'];

    $output .= '<td>' . $order['contact'] . '</td>';
    $arr[3] = $order['contact'];

    $output .= '<td>' . $order['company_name'] . '</td>';
    $arr[4] = $order['company_name'];

    $output .= '<td>' .  $restaurant_name.'</td>';
    $arr[5] = $restaurant_name;

    $output .= '<td>' . $order['total'] . " NIS" . '</td>';
    $arr[6] = $order['total'];   $totall  = $totall + $order['total'];

    $output .= '<td>' . $order['actual_total'] . " NIS" . '</td>';
    $arr[7] = $order['actual_total'];   $actual_total  = $actual_total + $order['actual_total'];

    $output .= '<td>' . $order['discount'] . " NIS" . '</td>';
    $arr[8] = $order['discount'];   $discount  = $discount + $order['discount'];

    $output .= '<td>' . $order['company_contribution'] . " NIS" . '</td>';
    $arr[9] = $order['company_contribution'];

    $output .= '<td>' .  $order['payment_info'].'</td>';
    $arr[10] = $order['payment_info'];

    $output .= '<td>' .  $order['order_status'].'</td>';
    $arr[11] = $order['order_status'];

    if (empty($order['transaction_id'])) {
        $order['transaction_id'] = "N/A";
    }
    $output .= '<td>' . $order['transaction_id'] . '</td>';
    $arr[12] = $order['transaction_id'];
